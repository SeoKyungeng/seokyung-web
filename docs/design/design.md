...
| 설비현황 | 데이터 중심 + 수평 스크롤 쇼케이스 | Technical |
...
| 설비 카드 | 스크롤 진입 시 scale(0.95→1) + opacity | `power2.out` | 0.6s |
...
| **비대칭 2컬럼** (5:7 또는 7:5) | 장비 상세 |
| **풀너비 중앙 텍스트** | CEO 인사말 |
| **3컬럼 카드 그리드** | 경영이념, ESG 카드 |
...
| 패턴 | 스타일 | 용도 |
|------|--------|------|
| 스켈레톤 (카드) | `smoke` 배경 + 펄스 애니메이션 (opacity 0.5↔1, 1.5s) | 설비 카드, ESG 카드 |
